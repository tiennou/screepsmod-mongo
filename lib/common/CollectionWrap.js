...
class CollectionWrap {
  ...
  async update (query, doc, opts = {}) {
    ...
    if (isOperatorUpdate) {
      ...
      // Omit _id from replacements: the filter already uses ObjectId (see _patchQuery),
      // while find() results often have _id as hex strings. Newer MongoDB rejects
      // replaceOne when replacement _id does not exactly match the stored value (code 66).
      const replacement =
        doc && typeof doc === 'object' && !Array.isArray(doc)
          ? Object.assign({}, doc)
          : doc
      if (replacement && typeof replacement === 'object' && replacement._id !== undefined) {
        delete replacement._id
      }
      res = await this.col.replaceOne(query, replacement, mongoOpts)
    }
