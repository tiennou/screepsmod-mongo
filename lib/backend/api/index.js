...
/**
 * Like legacy `q-json-response`, envelopes success as `{ ok: 1, ...data }`.
 * Unlike it, does NOT call next() after responding: our routes shadow vanilla
 * backend routes at the same paths, and falling through would execute the
 * vanilla handler on an already-answered request (double side effects, and
 * ERR_HTTP_HEADERS_SENT crashes in its tokenAuth).
 */
...
const auth = serverRequire('@screeps/backend/lib/game/api/auth')
const authlib = serverRequire('@screeps/backend/lib/authlib')
